...
2. **なぜ存在するか** / 対象者 / 差別化点。目的が立っていないと、後続の機構・実行条件・機能の
   記述は読者に何も入ってこない — what・how・who に答えて **why** だけ欠ける README は
   「誰の役にも立たないのに何故ここで動き続けているのか」を訪問者に残す（CA 2026-08-20 著者指摘）。
   **これは節名・見出しのテンプレではない**（「Why this exists」節を置けという意味ではなく、
   identity 文に溶けていても lead の一文でもよい）。判定は「詳細に入る前に、この repo が何のために
   あるかを読者が言えるか」だけ。道具でない repo（実験装置・観察計器・研究 apparatus）は
   **道具でないと明言し、産物が何か**（履歴・データ・記録）を言う。対象者の文を「〜向けのツール」
   の形で書かない。外部 platform で動く repo は、その platform が**目的か場か**を一言で区別する
...
- **at-a-glance（If you came for… → Start at）表は既定で置かない。** 有効性の根拠がなく、行ラベルに
  内部語が入ると第一画面の文脈密度を上げるだけになる（2026-08-19 の棚卸しで規約から外した）。
...
- **日本語 README の地の文はですます調**。`writing-ecosystem` の Voice 規約（だ/である）からの
  **意図的な分岐** — README は「初対面の案内」で、である調の断定連打は入口では威圧になる。将来の
  stocktake がこの差を「不整合」として逆修正しないこと。表のセル・体言止め・見出しは適用外。
  英語 README は一人称のパーソナルな register を保つ
- **造語は削除せず、初出に一行の平易な言い換え**（造語は graph.jsonld / glossary と連合する引用
  アンカー）。**README 全体で造語は ~6 個まで**、1 文に 2 個以上を保持させない、lead は造語密度を
  最小に（使ってよいのはタイトルが既に約束している語だけ）。**段落は役割ごとに割る**（1 段落 1 役割）
- **ADR 番号・他 repo・evidence ファイルへの参照は導線であって説明の代替ではない** — 参照を消しても
  文が意味を運ぶこと。内部史（version・モデル交代・「7 → 2」）と生数値（閾値・効果量）は、読者の
  問いに答えるときだけ。それ以外は docs/ へ
...
- **span 編集は追い越した段落を刈る** — 新段落が旧段落の主張を吸収したら旧段落を残さない。第一画面の
  同一主張 2 回は判定器が継ぎ足し痕（K2）として最初に拾う（CA 2026-08-20 実測）
...
- 証拠 JSON の件数で verdict を決める（「ADR 28 箇所だから Fix」）/ 逆に regex で意味判断を代用する
- AI surface の数値指標（ski-ramp / entity density）を人間 README に流用する
- 「ビジュアル優先」を散文の画像化と解釈する（raster 図は text-only LLM に不可視）
- 「削るな・再構造化せよ」を効かせすぎて偽装 llms-full.txt 化する
- 機械層（llms.txt / graph）が backstop する前提で README フロアを薄くする
- 造語を削って名前ごと消す（gloss を足すのが正解）/ 逆に造語を 6 個超えて積む
- ADR 番号を説明の代替にする（「（ADR-0012）」が文の唯一の根拠）
